...
public class StagesActions extends TestBase {
	...
	@FindBy(xpath = "//tr[1]//td[6]//img[@title='stage_settings_list']")
	public WebElement stageSettingListOptions;

	@FindBy(xpath = "//th[text()='Stage settings']")
	public WebElement verifyStageSetiings;
	...
	@FindBy(xpath = "//td[1]//a")
	public List<WebElement> stageSettingsList;
	...
	String stagesActionS_FilePath = "C:\\Users\\W2378\\git\\Advaita\\Advaita_TDD\\src\\main\\resources\\stagesActions.properties";
...
//		unWait(1);
	...
	public StagesActions evaluationFilter(List<String> evaluationFieldOptions, String stageName) {
		selectAndClickOptions(Arrays.asList(multiSelectDropdownFrom, datePickerDropdown, dateRangerDropdown,
...
//	Write Code for SOP Delete
	...
	public StagesActions reportFields() {
		selectOptionsInMultiSelect(multiSelectDropdownFrom, reportFieldsOptions);
		saveRecord();
...
//============================================================================================================================
	...
	public void naviagtoStageActions(String EnterStageName) {

		assertTrue(searchTextfield.isDisplayed(), "searchTextfield is nOt Displayed");

		searchTextfield.sendKeys(EnterStageName);

		searchButton.isDisplayed();

		searchButton.click();

		assertTrue(stageSettingListOptions.isDisplayed(), "stageSettingListOptions is Not Displayed");
		stageSettingListOptions.click();

		assertTrue(verifyStageSetiings.isDisplayed(), "Failed : Site setting page is not Displayed");

	}

	public void chooseStagesActionOption(String stageSettingsActions) throws IOException {

		// PropertieFileUtil.extractAllAndStore(stagesActionS_FilePath,
		// stageSettingsList, "stageSettingListActions");
//
//		// Step 1: Find all rows in the table
//		List<WebElement> rows = driver.findElements(By.xpath("//tbody/../..//table//tr"));
//
//		boolean found = false;
//
//		for (WebElement row : rows) {
//			try {
//				// Step 2: Get the Stage Name text from the first <td>
//				String stageName = row.findElement(By.xpath("./td[1]")).getText().trim();
//
//				// Step 3: Match with target name
//				if (stageName.equalsIgnoreCase(targetStageName.trim())) {
//
//					// Step 4: Click on the "Settings" icon inside this row's last column
//					WebElement settingsIcon = row.findElement(By.xpath(
//							".//td[last()]//i[contains(@class, 'fa-cog') or contains(@class,'fa-gear') or contains(@class,'settings')]"));
//					settingsIcon.click();
//					System.out.println("Clicked Settings for Stage: " + stageName);
//					found = true;
//					break;
//				}
//
//			} catch (Exception e) {
//				System.out.println("Issue with row: " + e.getMessage());
//			}
//		}
//
//		if (!found) {
//			System.out.println("Stage '" + targetStageName + "' not found!");
//		}

	}

	public void chooseStagesActionOption() throws IOException {

		String reportFileds = PropertieFileUtil.getSingleTextFromPropertiesFile(stagesActionS_FilePath,
				"stageSettingsActions9");

		PropertieFileUtil.clickIfMatch(stageSettingsList, reportFileds);
		
		//DropDown.selectMultipleOptionsWithCustomSelection(driver, , null, 0);
