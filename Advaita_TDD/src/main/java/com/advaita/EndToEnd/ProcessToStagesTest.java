...
public class ProcessToStagesTest extends TestBase {
	...
	String process = PropertieFileUtil.getSingleTextFromPropertiesFile(PROPERTIES_FILE_PATH2,"process") + lastName1();
	String subProcess = PropertieFileUtil.getSingleTextFromPropertiesFile(PROPERTIES_FILE_PATH2,"subProcess") + lastName1();
	String subSubProcess = PropertieFileUtil.getSingleTextFromPropertiesFile(PROPERTIES_FILE_PATH2,"subSubProcess") + lastName1();
	String stageName = PropertieFileUtil.getSingleTextFromPropertiesFile(PROPERTIES_FILE_PATH2,"stage") + lastName1();
	...
	public void processToStage() throws Throwable {
		...
		dataset.createDataset(datasetName, process, subProcess, subSubProcess)
				.addFieldNames(0, "Name of the Customer", character, "100", "1").addRow()
				.addFieldNames(1, "Phone Number", number, "14", "1").addRow()
				.addFieldNames(2, "Email", character, "100", "1").addRow()
				.addFieldNames(3, "Location", character, "100", "1").datasetCreate();
		metaData.metaDataCreate(metaDataName, process, subProcess, subSubProcess, datasetName).execute(metaDataName);
		manualUpload.manualUploadRecord(manualUploadName, datasetName, uploadFileName, "uploaded", fileName, "Sheet1");
		nonMeasurableSetPage.NavToNonMeasurableTablePage().createQuestionSetName(nonMeasurableSetName1)
				.dropdownAndMultiSelect(dropDown, 1, 1, "Review Status", reviewDropdownOptions, "yes", "yes", "yes")
				.addQuestion(1).textBox(textBox, 1, 2, "Remarks", "yes", "yes", "5", "200", "All", "yes")
				.saveRecordAndBack();

		measurableSetPage.NavToMeasurableTablePage().createQuestionSetName(measurableSetName1)

				.textBox(textBox, 1, 1, "Name", "yes", "no", "2", "100", "All", "yes").addQuestion(1)
				.textBox(textBox, 1, 2, "Email ID", "yes", "no", "2", "100", "Is Email", "yes").addQuestion(1)
				.radioButton(radioButton, 1, 3, "Gender", "yes", "no", radioButtonOptions).saveRecordAndBack();

		stages.navigateToStageCreate().stageNameAndProcess(stageName, process, subProcess, subSubProcess)
				...
				.nonMeasurableDropdown("Section D", nonMeasurableSetName1).actionSection("all")
//                .actionSectionToggle("all")
				.dispositionSection().saveAndConfirmation();
		stagesActions.navtoStages().clickOnStageHistoryField(stageName, evaluationFields)
				.escalationFields(sectionBCheckBoxes);
		;
		samplingPlanAndGenerationPage.navToCreate().createSampling(process, subProcess, subSubProcess, stageName,
				samplingName, samplingTo, totalCall, allSampleCondition, needCondition, needSampling);

		manualAllocationPage.manualAllocationData(process, subProcess, subSubProcess, stageName, "Role Wise", "All",
				"Capture_admin");
	...
	public void userCreation(String firstName, String lastName, String roleName) {
		...
		userSetupPage.userCreationFields(firstName, lastName, "Qwerty@123").singleGroupSelect(roleName)
	...
	public Object[][] userCreation() {
		...
		return new Object[][] { { "James", "Agent", "Agent" }, { "James", "Team Lead", "Team Lead" },
				{ "James", "Admin", "Admin" } };
	}
