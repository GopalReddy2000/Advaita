...
public class CommonUtils extends TestBase {
	...
	public static void scrollToBottom(WebDriver driver) {
        ...
        while (true) {
            js.executeScript("window.scrollTo(0, document.body.scrollHeight)");
            try {
                Thread.sleep(500); // Wait for potential dynamic content to load
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
            long newHeight = (long) js.executeScript("return document.body.scrollHeight");
            if (newHeight == lastHeight) {
                break; // Exit when no more scrolling is possible
            }
            lastHeight = newHeight;
        }
    }

	public void clickOutSideTheDropDown(WebElement element) throws Throwable {
		Thread.sleep(1000); // Optional wait to observe
		Point location = element.getLocation();
		int elementX = location.getX();
		int elementY = location.getY();
		robot.mouseMove(elementX + 10, elementY + elementY); // +10 to avoid clicking on the edge
